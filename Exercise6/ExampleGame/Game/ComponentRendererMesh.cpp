...
void ComponentRendererMesh::Init(rapidjson::Value& serializedData) {
	...
	_mesh1 = sre::Mesh::create()
        .withPositions(positions1)
        .withUVs(uvs1)
        .withIndices(idxs, sre::MeshTopology::Triangles, 0)
        .build();

    _mesh2 = sre::Mesh::create()
        .withPositions(positions2)
        .withUVs(uvs1)
        .withIndices(idxs, sre::MeshTopology::Triangles, 0)
        .build();

    _mesh3 = sre::Mesh::create()
        .withPositions(positions3)
        .withUVs(uvs1)
        .withIndices(idxs, sre::MeshTopology::Triangles, 0)
        .build();

    _material = sre::Shader::getUnlit()->createMaterial();
    _material1 = sre::Shader::getUnlit()->createMaterial();
    ...
    _material1->setTexture(_texture);
}
...
void ComponentRendererMesh::Render(sre::RenderPass& renderPass) {
    ...
    renderPass.draw(_mesh1, gameObject->GetTransform(), _material1);
	renderPass.draw(_mesh2, gameObject->GetTransform(), _material1);
	renderPass.draw(_mesh3, gameObject->GetTransform(), _material1);
